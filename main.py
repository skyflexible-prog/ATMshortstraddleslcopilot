...
class DeltaExchangeAPI:
    ...
    def get_btc_spot_price(self) -> float:
        """Get current BTC spot price from BTCUSD perpetual ticker"""
        try:
            # Get BTCUSD perpetual ticker which contains spot_price
            response = self.make_request("GET", "/v2/tickers/BTCUSD")
            logger.info(f"BTC ticker response: {response}")
            
            if response.get('success') and response.get('result'):
                ticker_data = response['result']
                # Try to get spot_price from ticker data
                if 'spot_price' in ticker_data:
                    spot_price = float(ticker_data['spot_price'])
                    logger.info(f"Found BTC spot price: {spot_price}")
                    return spot_price
                else:
                    # Fallback to mark_price if spot_price not available
                    mark_price = float(ticker_data.get('mark_price', 0))
                    logger.info(f"Using BTC mark price as fallback: {mark_price}")
                    return mark_price
            
            raise Exception("BTC spot price not found in ticker response")
    ...
    def find_atm_strikes(self, spot_price: float, options_chain: List[Dict]) -> Tuple[Optional[Dict], Optional[Dict]]:
        ...
        if not options_chain:
            logger.error("Empty options chain provided")
            return None, None
        ...
        # First pass: find the strike closest to spot price
        for option in options_chain:
            try:
                strike_price = float(option.get('strike_price', 0))
                if strike_price > 0:  # Valid strike price
                    diff = abs(strike_price - spot_price)
                    if diff < min_diff:
                        min_diff = diff
                        atm_strike = strike_price
            except (ValueError, TypeError) as e:
                logger.warning(f"Invalid strike price in option: {option}")
                continue
        
        if atm_strike is None:
            logger.error("Could not find valid ATM strike")
            return None, None
            
        logger.info(f"ATM strike selected: {atm_strike} (spot: {spot_price})")
        
        # Second pass: find call and put options at ATM strike
        for option in options_chain:
            try:
                if float(option.get('strike_price', 0)) == atm_strike:
                    contract_type = option.get('contract_type', '')
                    if 'call_options' in contract_type:
                        call_option = option
                        logger.info(f"Found call option: {option.get('symbol')}")
                    elif 'put_options' in contract_type:
                        put_option = option
                        logger.info(f"Found put option: {option.get('symbol')}")
    ...
    def place_stop_loss_order(self, product_id: int, side: str, stop_price: str, size: int = 1) -> Dict:
        ...
        try:
            logger.info(f"Placing stop loss order: {order_data}")
            response = self.make_request("POST", "/v2/orders", data=json.dumps(order_data))
            logger.info(f"Stop loss order response: {response}")
            return response
        except Exception as e:
...
class TradingBot:
    ...
    async def execute_short_straddle(self, chat_id: int, bot: Bot) -> str:
        ...
        try:
            ...
            logger.info("Getting BTC spot price...")
            ...
            logger.info(f"Getting options chain for expiry: {expiry_date}")
            options_chain = self.delta_api.get_options_chain(expiry_date)
            
            if not options_chain:
                error_msg = f"❌ No options found for today's expiry ({expiry_date}). Please check if options are available for today."
                logger.error(error_msg)
                return error_msg
            
            logger.info(f"Found {len(options_chain)} options")
            
            # Find ATM call and put options
            call_option, put_option = self.delta_api.find_atm_strikes(spot_price, options_chain)
            
            if not call_option or not put_option:
                error_msg = "❌ Could not find ATM call and put options. This might be due to no same-day expiry options available."
                logger.error(error_msg)
                return error_msg
            ...
            logger.info("Placing call option sell order...")
            ...
            logger.info("Placing put option sell order...")
            ...
            if not call_order.get('success') or not put_order.get('success'):
                error_msg = f"❌ Failed to execute short straddle orders.\nCall order: {call_order}\nPut order: {put_order}"
                logger.error(error_msg)
                return error_msg
            ...
            logger.info("Placing call stop loss order...")
            ...
            logger.info("Placing put stop loss order...")
            ...
            result_message = (
                ...
                f"🛡️ Stop Loss Orders: {'✅' if call_sl_order.get('success') and put_sl_order.get('success') else '⚠️'}\n"
                f"⏰ Expiry: {expiry_date}\n\n"
                f"⚠️ WARNING: This is a high-risk strategy with unlimited loss potential!"
            )
            
            logger.info("Short straddle execution completed successfully")
            return result_message
            
        except Exception as e:
            error_msg = f"❌ Error executing short straddle: {str(e)}"
            logger.error(error_msg)
            return error_msg
