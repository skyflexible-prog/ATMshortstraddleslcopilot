...
class DeltaExchangeAPI:
    ...
    def place_order(self, product_id: int, side: str, size: int = 1, order_type: str = "market_order") -> Dict:
        ...
        try:
            response = self.make_request("POST", "/v2/orders", data=str(order_data).replace("'", '"'))
            return response
        except Exception as e:
...
class TradingBot:
    ...
    async def execute_short_straddle(self, chat_id: int, context: ContextTypes.DEFAULT_TYPE) -> str:
        """Execute short straddle strategy"""
        try:
            # Get current BTC spot price
            spot_price = self.delta_api.get_btc_spot_price()
            await context.bot.send_message(chat_id, f"📊 Current BTC Spot Price: ${spot_price:,.2f}")
            ...
            await context.bot.send_message(chat_id, "🔄 Executing Short Straddle...")
            ...
            await context.bot.send_message(chat_id, "🛡️ Placing Stop Loss Orders...")
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == 'execute_straddle':
        ...
        result = await trading_bot.execute_short_straddle(query.message.chat_id, context)
        
        keyboard = [[InlineKeyboardButton("🔙 Back to Menu", callback_data='back_to_menu')]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        await context.bot.send_message(query.message.chat_id, result, reply_markup=reply_markup)
